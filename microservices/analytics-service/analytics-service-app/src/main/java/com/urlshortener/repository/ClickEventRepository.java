...
public interface ClickEventRepository extends JpaRepository<ClickEvent, Long> {
    ...
    @Query("SELECT COUNT(c) FROM ClickEvent c WHERE c.urlId = ?1")
    Long countByUrlId(Long urlId);

    @Query("SELECT c.country, COUNT(c) FROM ClickEvent c WHERE c.urlId = ?1 GROUP BY c.country")
    List<Object[]> getClicksByCountry(Long urlId);

    @Query("SELECT c.browser, COUNT(c) FROM ClickEvent c WHERE c.urlId = ?1 GROUP BY c.browser")
    List<Object[]> getClicksByBrowser(Long urlId);

    @Query("SELECT c.deviceType, COUNT(c) FROM ClickEvent c WHERE c.urlId = ?1 GROUP BY c.deviceType")
